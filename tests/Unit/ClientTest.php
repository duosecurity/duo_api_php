...
class ClientTest extends PHPUnit_Framework_TestCase {
    ...
    public function testUrlEncodeParametersSimple() {
        ...
        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($simple));
    ...
    public function testUrlEncodeParametersZero() {
        ...
        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($zero));
    ...
    public function testUrlEncodeParametersOne() {
        ...
        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($one));
    ...
    public function testUrlEncodeParametersPrintableAscii() {
        ...
        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($printable));
    ...
    public function testUrlEncodeParametersCommonPrefix() {
        ...
        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($common));

        $this->assertEquals('foo=1&foo_bar=2', $result);
    }

    public function testUrlEncodeParametersUnicodeValues() {
        $make_unicode = function ($s) { return json_decode('"' . $s . '"'); };
        $unicode = array(
            'bar' => $make_unicode('\u2815\uaaa3\u37cf\u4bb7\u36e9\ucc05\u668e\u8162\uc2bd\ua1f1'),
            'baz' => $make_unicode('\u0df3\u84bd\u5669\u9985\ub8a4\uac3a\u7be7\u6f69\u934a\ub91c'),
            'foo' => $make_unicode('\ud4ce\ud6d6\u7938\u50c0\u8a20\u8f15\ufd0b\u8024\u5cb3\uc655'),
            'qux' => $make_unicode('\u8b97\uc846-\u828e\u831a\uccca\ua2d4\u8c3e\ub8b2\u99be'),
        );

        $urlEncodeParameters = self::getMethod('urlEncodeParameters');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $urlEncodeParameters->invokeArgs($client, array($unicode));
    ...
    public function testUrlEncodeParametersUnicodeKeysValues() {
        $make_unicode = function ($s) { return json_decode('"' . $s . '"'); };
        $unicode = array(
            $make_unicode("\u469a\u287b\u35d0\u8ef3\u6727\u502a\u0810\ud091\u2815\uc170") => $make_unicode("\u0f45\u1a76\u341a\u654c\uc23f\u9b09\uabe2\u8343\u1b27\u60d0"),
            $make_unicode("\u7449\u7e4b\uccfb\u59ff\ufe5f\u83b7\uadcc\u900c\ucfd1\u7813") => $make_unicode("\u8db7\u5022\u92d3\u42ef\u207d\u8730\uacfe\u5617\u0946\u4e30"),
            $make_unicode("\u7470\u9314\u901c\u9eae\u40d8\u4201\u82d8\u8c70\u1d31\ua042") => $make_unicode("\u17d9\u0ba8\u9358\uaadf\ua42a\u48be\ufb96\u6fe9\ub7ff\u32f3"),
            $make_unicode("\uc2c5\u2c1d\u2620\u3617\u96b3F\u8605\u20e8\uac21\u5934") => $make_unicode("\ufba9\u41aa\ubd83\u840b\u2615\u3e6e\u652d\ua8b5\ud56bU"),
        );
    ...
    public function testCanonicalize() {
        ...
        $canonicalize = self::getMethod('canonicalize');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $canonicalize->invokeArgs($client, ...array($canon));
    ...
    public function testSignature() {
        ...
        $sign = self::getMethod('sign');
        $client = new DuoAPI\Client("TEST", "TEST", "TEST");
        $result = $sign->invokeArgs($client, ...array($args));
